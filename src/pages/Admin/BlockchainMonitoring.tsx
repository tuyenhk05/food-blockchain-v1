...
import { BlockchainTransactionModal } from "../../components/Common/BlockchainTransactionModal";
export function BlockchainMonitoring() {
  const [selectedTx, setSelectedTx] = useState<any>(null);
  const [searchTerm, setSearchTerm] = useState("");
  const [isRefreshing, setIsRefreshing] = useState(false);
  // Flatten all transactions from mock data
  const transactions = mockBatches
    .flatMap((batch) =>
      batch.stages
        .filter((stage) => stage.blockchain.verified)
        .map((stage) => ({
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`p-2 text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-all ${
                isRefreshing
                  ? "animate-spin text-emerald-600 border-emerald-200"
                  : ""
              }`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="col-span-1 md:col-span-2 bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex items-center gap-4">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Tìm kiếm theo Hash, Mã Lô Hàng hoặc Giai Đoạn..."
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className="font-mono text-sm text-gray-600 group-hover:text-blue-600 transition-colors cursor-pointer"
